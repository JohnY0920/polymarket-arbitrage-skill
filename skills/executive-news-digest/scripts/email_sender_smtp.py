#!/usr/bin/env python3
"""
Email Sender for Executive News Digest (SMTP Version)
Sends formatted digest via Gmail SMTP
"""
...
<body>
    ...
    def send_email(self, english_content: Dict, chinese_content: Dict) -> bool:
        """Send the digest via email using SMTP"""
        try:
            # Create message
            msg = MIMEMultipart('alternative')
            msg['From'] = self.sender_email
            msg['To'] = self.recipient_email
            msg['Subject'] = f"📰 Executive News Digest - {datetime.now().strftime('%b %d, %Y')}"
            ...
            # Send via SMTP
            print(f"Sending email to {self.recipient_email} via SMTP...")
...
def main():
    ...
    sender = EmailSenderSMTP(
        sender_email=os.environ.get('GOG_ACCOUNT'),
